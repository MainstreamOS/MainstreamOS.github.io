...
A tiling Linux desktop is normally built by hand, in text files. Mainstream OS is built by clicking, making your choices inside a single settings app — not a config file, and never a terminal. It comes with a full theming system: the interface recolors itself around your wallpaper, and that — along with every other change you make — saves as a complete theme you can swap or share whenever you like. And one keypress hands the whole machine to a Steam Deck-style gaming session.
...
- **SteamOS-style Gaming Mode** — Press `Super+G` and the whole desktop swaps for a full gamescope Steam Big Picture session — the same session model SteamOS uses — and one click brings the desktop back. AMD, Intel, and NVIDIA alike.
- **Whole-desktop Material You** — Pick any wallpaper and the system recolors from it — the shell, the Settings app, your terminal, your apps, and the lock screen. The desktop changes as you watch; anything already open catches up the next time you start it.
...
- **Title bars, on or off** — On for a familiar desktop, off for pure tiling. It takes effect instantly.

## Making it yours

- **Theme system** — A theme is your whole look — wallpaper, colors, app style, icons, interface changes, and window styling — saved under a name with a preview. Switch between saved themes in one tap, pair a Day and Night theme that follow Night Light or hours you set, or export a theme to a single file and import it on another computer.
...
- **Install self-check** — After every install, a self-verification pass runs and writes you a health report, so a bad install tells you instead of failing quietly.
